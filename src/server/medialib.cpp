...
static struct channel_context_st* path2entry(const char* path){
	...
	me = (channel_context_st*)malloc(sizeof(*me));
	if(me == NULL){
	...
	if(glob(pathstr, 0, NULL, &me->mp3glob) != 0){
		syslog(LOG_ERR, "%s is not a channel dir (Can't find mp3 files).");
...
int mlib_getchnlist(struct mlib_listentry_st** result, int* resnum){
	...
	ptr = (mlib_listentry_st*)malloc(globres.gl_pathc*sizeof(channel_context_st));
	if(ptr == NULL){
		perror("malloc()");
		exit(1);
	}
	...
	*result = (mlib_listentry_st*)realloc(ptr, num*sizeof(struct mlib_listentry_st));
	*resnum = num;
	return 0;
}
